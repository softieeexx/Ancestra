...
// src/AncestraFactory.sol

contract AncestraFactory is IAncestraFactory {
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        require(tokenA != tokenB, "Ancestra: IDENTICAL_ADDRESSES");
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), "Ancestra: ZERO_ADDRESS");
        require(getPair[token0][token1] == address(0), "Ancestra: PAIR_EXISTS");

        bytes memory bytecode = type(AncestraPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        AncestraPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external override {
        require(msg.sender == feeToSetter, "Ancestra: FORBIDDEN");
        feeTo = _feeTo;
    }
